...
int main( int t_narg, char **t_args )
{
    ...
    while ( 1 )
    {
        ...
        int poll_res = poll( l_read_poll, 2, 5000 );
        ...
        {
            log_msg( LOG_DEBUG, "Poll timeout." );

            snprintf( l_buf, sizeof( l_buf ), "2+2\n");

            int l_len = write(l_sock_server, l_buf, strlen(l_buf));
            if (l_len < 0)
            {
                log_msg(LOG_ERROR, "Unable to send test data to server.");
                break;
            }
            memset(l_buf, 0, sizeof(l_buf));

            snprintf( l_buf, sizeof( l_buf ), "4-2\n");

            l_len = write(l_sock_server, l_buf, strlen(l_buf));
            if (l_len < 0)
            {
                log_msg(LOG_ERROR, "Unable to send test data to server.");
                break;
